...
export function transitionIn(el, show, component, forceSkip = false) {
    ...
    if (el.__x_transition && el.__x_transition.type === 'in') {
        // there already a similar transition going on, this was probably triggered by
        // an different property, let's just leave the previous one doing its job
        return
    }
...
export function transitionOut(el, hide, component, forceSkip = false) {
    ...
    if (el.__x_transition && el.__x_transition.type === 'out') {
        // there already a similar transition going on, this was probably triggered by
        // an different property, let's just leave the previous one doing its job
        return
    }
...
export function transitionHelperIn(el, modifiers, showCallback) {
    ...
    transitionHelper(el, modifiers, showCallback, () => {}, styleValues, 'in')
}
...
export function transitionHelperOut(el, modifiers, settingBothSidesOfTransition, hideCallback) {
    ...
    transitionHelper(el, modifiers, () => {}, hideCallback, styleValues, 'out')
}
...
export function transitionClassesIn(el, component, directives, showCallback) {
    ...
    transitionClasses(el, enter, enterStart, enterEnd, showCallback, () => {}, 'in')
}
...
export function transitionClassesOut(el, component, directives, hideCallback) {
    ...
    transitionClasses(el, leave, leaveStart, leaveEnd, () => {}, hideCallback, 'out')
}
